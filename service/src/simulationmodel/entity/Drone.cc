...
void Drone::getNextDelivery() {
  if (!package && model && model->scheduledDeliveries.size() > 0) {
    auction = new Auctioneer();
    auction->calculateBids(model);
  }
  if (package) {
...
void Drone::update(double dt) {
  ...
  if (toPackage) {
    ...
    if (toFinalDestination->isCompleted()) {
      ...
      UnattendedPackage* p = dynamic_cast<UnattendedPackage*>(package);

      p->findOwner(model);
      p->findPirates(model);
      p->notifyOwner(position, package);
